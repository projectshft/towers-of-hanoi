const Board = () => {
  const playGame = (pegs, discs) => {
    ...
    // ***************** PLAY GAME ********************* //
    while (!gameWon) {
      let startPeg = parseInt(
        prompt(`Which peg would you like to remove a disc from?`)
      );
      let endPeg = parseInt(
        prompt(`Which peg would you like to move the disc to?`)
      );
    ...
    let playAgain = prompt(
      `You won the game in ${numMoves} moves. Would you like to play again? Y/n`
    );
