...
const printBoard = (array) =>
  array.map((item) => console.log(`--- ${item.join(' ')}`));
...
const boardObj = {
  ...
  moveDisc(startPeg, destPeg) {
    ...
    if (
      !boardObj.hasOwnProperty(startPeg) ||
      !boardObj.hasOwnProperty(destPeg)
    ) {
      console.log('Peg does not exist.');

      // make sure last el of startPeg is < less than last el of end peg
    } else if (
      boardObj[startPeg][boardObj[startPeg].length - 1] >
      boardObj[destPeg][boardObj[destPeg].length - 1]
    ) {
      console.log(
