...
var Board = function () {
  ...
  var legalMoves = function (oldPeg, newPeg) {
    var discNum = board[oldPeg-1][board[oldPeg-1].length-1]     //find value of selected disc

    var legal = board.filter (function(num, index, array) {     //filter any empty pegs
      if (array[index].length == 0) {
        return true;
      }
    });
    last = [];
    var findLasts = board.forEach(function (num) {      //find disc value for each peg
      var index = num[num.length - 1]
      return (last.push(index))
    });

    var check = function (discNum, newPeg) {        //if disc value is smaller than peg selcted, valid move
  ...
  var moveDisc = function (oldPeg, newPeg) {
    ...
    var newPeg = newPeg - 1;
    legalMoves(discNum, newPeg);                    //2. check if newPeg is in legalMoves array
